...
module mint_nft::create_nft_with_resource_account {
    ...
    struct TokenDataCustom has key
    {
        token_id:TokenId,
    }
    ...
    fun init_module(resource_signer: &signer) {
        let collection_name = string::utf8(b"Shivprasad Collection");
        let description = string::utf8(b"Description");
        let collection_uri = string::utf8(b"Collection uri");
        let token_name = string::utf8(b"ShivToken");
        let token_uri = string::utf8(b"https://i.pinimg.com/736x/1b/06/77/1b067738b18d907ae7105aa378af3bd9.jpg");
        ...
        let token_data_id = token::create_tokendata(
            ...
            collection_name,
            token_name,
            string::utf8(b"This is my nft old"),
            0,
            token_uri,
    ...
    public entry fun mint_event_ticket(receiver: &signer) acquires ModuleData {
        ...
        token::mutate_token_properties(
            ...
            0,
            1,
            vector<String>[string::utf8(b"BACKGROUND"),string::utf8(b"SHIRT"),string::utf8(b"EYE")],
            vector<vector<u8>>[bcs::to_bytes(&signer::address_of(receiver)),bcs::to_bytes(&signer::address_of(receiver)),bcs::to_bytes(&signer::address_of(receiver))],
            vector<String>[string::utf8(b"address"),string::utf8(b"address"),string::utf8(b"address")],
        );
    ...
    // public entry fun update_properties(receiver:&signer) acquires ModuleData
    // {
    //     let module_data = borrow_global_mut<ModuleData>(@mint_nft);
    //     let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
    ...
    //         0,
    //         2,
    //         vector<String>[string::utf8(b"P1"),string::utf8(b"p2"),string::utf8(b"p3")],
    //         vector<vector<u8>>[bcs::to_bytes(&signer::address_of(receiver)),bcs::to_bytes(&signer::address_of(receiver)),bcs::to_bytes(&signer::address_of(receiver))],
    //         vector<String>[string::utf8(b"address"),string::utf8(b"address"),string::utf8(b"address")],
    //     );
    // }
    ...
    //tested this function by calling from cli
    //Parameter type is string
    public entry fun update_description(description:String) acquires ModuleData,TokenDataCustom
    {
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        let token_data=borrow_global_mut<TokenDataCustom>(@mint_nft);
        let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
         let tempdata = token_data.token_id;
        let td= token::get_tokendata_id(tempdata);
        token::mutate_tokendata_description(&resource_signer,td,description);   
    }

    
    //Function to update  propertues of nft
    //tested this function by calling from cli
    //Parameter type are keys,values,types and all are vectors
    public entry fun update_property(keys: vector<string::String>, values: vector<vector<u8>>, types: vector<string::String>) acquires ModuleData,TokenDataCustom
    {
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        let token_data=borrow_global_mut<TokenDataCustom>(@mint_nft);
        let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
        let tempdata = token_data.token_id;
        let td= token::get_tokendata_id(tempdata);
        token::mutate_tokendata_property(&resource_signer,td,keys,values,types);   
    }
    //THis function is not entry because parameter type "Royalty" not supported for entry functions
    //To call this function we have to remove Royality type or need to crate Royalty inside functions or call it from another function
    public fun update_royalty(royalty: token::Royalty) acquires ModuleData,TokenDataCustom
    {
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        let token_data=borrow_global_mut<TokenDataCustom>(@mint_nft);
        let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
        let tempdata = token_data.token_id;
        let td= token::get_tokendata_id(tempdata);
        token::mutate_tokendata_royalty(&resource_signer,td,royalty);   
    }
    ...
    //tested this function by calling from cli
    //Parameter type is string
    public entry fun update_uri(uri:String) acquires ModuleData,TokenDataCustom
    {
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        let token_data=borrow_global_mut<TokenDataCustom>(@mint_nft);
        let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
        let tempdata = token_data.token_id;
        let td= token::get_tokendata_id(tempdata);
        token::mutate_tokendata_uri(&resource_signer,td,uri);   
    }

    
    //Function to update maximum token supply 
    //tested this function by calling from cli
    //Parameter type is int
    public entry fun update_maximum(maximum:u64) acquires ModuleData,TokenDataCustom
    {
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        let token_data=borrow_global_mut<TokenDataCustom>(@mint_nft);
        let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
        let tempdata = token_data.token_id;
        let td= token::get_tokendata_id(tempdata);
        token::mutate_tokendata_maximum(&resource_signer,td,maximum);   
    }
